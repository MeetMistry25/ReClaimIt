...
const FoundItemsDashboard = () => {
  ...
  const fetchFoundItems = async () => {
    try {
      ...
      const response = await fetch(`/api/items/found?${queryParams}`);
  ...
  const filteredItems = items.filter(item => {
    if (searchQuery) {
      ...
      return (
        ...
        item.placeFound.toLowerCase().includes(query)
      );
    }
    return true;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto dark:bg-gray-900 dark:text-gray-100">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-primary-100 mb-2">Found Items</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={filters.placeFound}
                  onChange={(e) => handleFilterChange('placeFound', e.target.value)}
                  className="input-field dark:bg-gray-900 dark:border-gray-700 dark:text-primary-100"
                />
              </div>
